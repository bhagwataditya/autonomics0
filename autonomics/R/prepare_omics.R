...
deconvolute_proteingroups <- function(
   ...
   if (nrow(swissprot)>0){
      ...
      swissprot  %>% magrittr::extract(, `PROTEIN-NAMES` %>% commonify_strings(), by = c('feature_id', 'GENES'))
   ...
   if (nrow(paralogs)>0){
      ...
      paralogs  %>% magrittr::extract(, `PROTEIN-NAMES` := `PROTEIN-NAMES` %>% commonify_strings(), by = 'feature_id')
