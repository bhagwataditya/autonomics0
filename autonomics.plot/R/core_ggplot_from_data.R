...
#' @param color_var           variable (samples or features depending on \code{MARGIN}) mapped to color aesthetic
#' @param fill_var            variable (samples or features depending on \code{MARGIN}) mapped to fill aesthetic
#' @param group_var           variable (samples or features depending on \code{MARGIN}) mapped to group aesthetic
#' @param linetype_var        variable (samples or features depending on \code{MARGIN}) mapped to linetype aesthetic
#' @param shape_var           variable (samples or features depending on \code{MARGIN}) mapped to shape aesthetic
#' @param size_var            variable (samples or features depending on \code{MARGIN}) mapped to size aesthetic
#' @param stroke_var          variable (samples or features depending on \code{MARGIN}) mapped to stroke aesthetic
#' @param weigth_var          variable (samples or features depending on \code{MARGIN}) mapped to weight aesthetic
#' @param facet1_var          variable (samples or features depending on \code{MARGIN}) for faceting (\code{ggplot2::facet_grid(facet1_var ~ .)})
#' @param facet2_var          variable (samples or features depending on \code{MARGIN}) for faceting (\code{ggplot2::facet_grid(. ~ facet2_var)})
...
# https://stackoverflow.com/a/14398674/2103880
   ...
   arg_list %<>%
      magrittr::extract(!sapply(arg_list, is.null))
...
# Process arguments & check prerequisites ---------------------------------
   ...
   if (n_obj > 1){
      ...
      {
         i %>%
            assertive.types::assert_is_any_of(c('eSet', 'EList', 'SummarizedExperiment', 'matrix'))
         if(MARGIN == 'samples')
         {
            assertive.base::assert_are_identical(ncol(object), ncol(i))
         } else {
            assertive.base::assert_are_identical(nrow(object), nrow(i))
         }
      }
   }
   ...
   localVars <- switch(
      MARGIN,
      samples  = autonomics.import::svars(object),
      features = autonomics.import::fvars(object))
   localNames <- switch(
      MARGIN,
      samples  = autonomics.import::snames(object),
      features = autonomics.import::fnames(object))
   localIdsToMark <- switch(
      MARGIN,
      samples  = autonomics.import::fdata(object) %>%
         magrittr::extract2('feature_id'),
      features = autonomics.import::sdata(object) %>%
   ...
   for(va in vars)
   {
      ...
      {
         va %>%
            assertive.sets::assert_is_subset(localVars)
   ...
   if(!is.null(marked_ids)){
      marked_ids %>%
         ...
         assertive.numbers::assert_all_are_in_range(
            lower = 0,
            upper = switch(
               MARGIN,
               samples  = nrow(object),
               features = ncol(object)))
      marked_ids <- localIdsToMark %>%
         magrittr::extract2(marked_ids)
   }
   ...
   scales %<>%
      match.arg(choices = c('fixed', 'free', 'free_x', 'free_y'), several.ok = FALSE)
...
# Combine and shape the data ----------------------------------------------
   ...
   for (va in stringi::stri_subset_regex(vars, pattern = '_var$')){
     ...
     if(length(ct) == 1)
     {
        ...
        for (i in seq_along(obj_list))
        {
           obj_list[[i]] %<>%
              cbind(
                 rep(ct[i], times = nrow(obj_list[[i]])) %>%
                    as.data.frame() %>%
                    magrittr::set_colnames(plotvar))
           # Insure original order
           obj_list[[i]][[plotvar]] %<>%
              factor(levels = ct)
        }
     }
   ...
   if(!is.null(marked_ids))
   {
      plotDF %<>%
         dplyr::mutate(
            marked_plotvar = feature_plotvar %in% marked_ids
         )
   }
...
# Generate basic plot object ----------------------------------------------
   ...
   {
      facetting_string <- c(
         ifelse('facet1_var' %in% vars, 'facet1_plotvar', '.'),
